...
def create_animation():
    ...
    def get_image(path):
        if path not in icon_images_cache:
            img = plt.imread(path)
            icon_images_cache[path] = OffsetImage(img, zoom=0.06)
    ...
    def update(frame):
        ...
        ax.set_title(f"ボイロキャラクター人気順位推移 (投稿数) ({int(years[year_idx])}年)", fontsize=28, pad=20)
        ax.set_xlabel("年", fontsize=18)
        ax.set_ylabel("順位 (投稿数)", fontsize=18)
    ...
    output_path = output_dir / "character_ranking_history_icons_count.mp4"
