...
def plot_nico_vs_voiro():
    ...
    ax1.set_ylabel('ニコニコ全体 投稿数', color='tab:blue', fontsize=18)
    ...
    ax1_twin.set_ylabel('ボイロ界隈 投稿数', color='tab:orange', fontsize=18)
    ax1_twin.tick_params(axis='y', labelcolor='tab:orange', labelsize=14)
    ax1_twin.set_yscale('log')

    ax1.set_title('年間投稿数の推移 (対数スケール比較)', fontsize=22)
    ...
    ax2.set_ylabel('ニコニコ全体 再生数', color='tab:green', fontsize=18)
    ...
    ax2_twin.set_ylabel('ボイロ界隈 再生数', color='tab:red', fontsize=18)
    ax2_twin.tick_params(axis='y', labelcolor='tab:red', labelsize=14)
    ax2_twin.set_yscale('log')

    ax2.set_title('年間再生数の推移 (対数スケール比較)', fontsize=22)
    ax2.grid(True, which="both", ls="-", alpha=0.2)
    ax2.set_xlabel('年', fontsize=18)
    ...
    ax2.legend(lines3 + lines4, labels3 + labels4, loc='upper center', fontsize=14)
    ...
    plt.title('ニコニコ全体に占めるボイロ界隈の割合推移', fontsize=22)
    plt.ylabel('シェア (%)', fontsize=18)
