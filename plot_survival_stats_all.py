...
def main():
    ...
    output_dir = Path("results/survival_analysis_with_travel")
    ...
    for label, series in all_survival.items():
        if label == "全体":
            color, linewidth, markersize, zorder, alpha = "tab:blue", 4, 10, 10, 1.0
        elif label == "旅行":
            color, linewidth, markersize, zorder, alpha = "tab:pink", 5, 12, 15, 1.0
        else:
            color, linewidth, markersize, zorder, alpha = "gray", 1.5, 5, 5, 0.4
            
        plt.plot(series.index, series.values, marker='s', label=label, color=color, 
                 linewidth=linewidth, markersize=markersize, zorder=zorder, alpha=alpha)
    plt.title("ジャンル別 投稿者生存曲線 (活動期間別) - 旅行強調", fontsize=22)
    ...
    for label, series in all_retirement.items():
        ...
        plt.plot(series.index, series.values, marker='o', label=label, color=color,
                 linewidth=linewidth, markersize=markersize, zorder=zorder, alpha=alpha)
    plt.title("ジャンル別 投稿者累積引退率 (0年目から表示) - 旅行強調", fontsize=22)
    plt.xlabel("活動期間 (年)", fontsize=18)
    ...
    for label, series in all_retirement.items():
        ...
        plt.plot(series_from_1.index, series_from_1.values, marker='o', label=label, color=color,
                 linewidth=linewidth, markersize=markersize, zorder=zorder, alpha=alpha)
    plt.title("ジャンル別 投稿者累積引退率 (1年目以降) - 旅行強調", fontsize=22)
    plt.xlabel("活動期間 (年)", fontsize=18)
    plt.ylabel("引退率 (%)", fontsize=18)
    plt.grid(True, linestyle="--", alpha=0.7)
    ...
    for label, series in all_continuation.items():
        ...
        plt.plot(series.index, series.values, marker='o', label=label, color=color,
                 linewidth=linewidth, markersize=markersize, zorder=zorder, alpha=alpha)
    plt.title("ジャンル別 デビュー年別投稿継続率 (現役率) - 旅行強調", fontsize=22)
